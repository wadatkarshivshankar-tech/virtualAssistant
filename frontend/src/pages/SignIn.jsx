...
const SignIn = () => {
  ...
  return (
    <div className='w-full h-screen bg-cover flex justify-center items-center' style={{backgroundImage:`url(${bg})`}}>
        ...
        <form className='w-[90%] h-150 max-w-125 bg-[#00000062] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-5 px-5' onSubmit={handleSignIn}>
          <h1 className='text-white text-[30px] font-semibold mb-7.5'>Sign in to<span className='text-blue-400'>Virtual Assistant</span></h1>
          ...
          <button className='min-w-[150px] h-[55px] mt-[25px] text-black font-semibold  bg-white rounded-full text-[19px] ' disabled={Loading}>{Loading?"Loading...":"Sign In"}</button>

          <p className='text-[white] text-[18px] cursor-pointer' onClick={()=>navigate("/signup")}>Register to new account ? <span className='text-blue-400'>Sign Up</span></p>
